...
export default function MonthDetailsModal({ monthId, items, counts, onClose }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[70] bg-n-900/40 flex items-end md:items-center justify-center p-0 md:p-4"
      onClick={onClose}
    >
      <div
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-n-200">
          ...
          <button
            onClick={onClose}
            className="text-n-400 hover:text-n-700 text-2xl leading-none px-2"
          >
            ×
          </button>
        </div>

        <div className="px-4 py-3 flex items-center gap-6 border-b border-n-200">
        ...
        <div className="overflow-y-auto px-4 py-3 space-y-4">
          {cats.length === 0 && (
            <p className="text-n-500 text-center py-6">{t("noClosing")}</p>
          )}
          {cats.map((cat) => (
            <div key={cat}>
              <h4 className="text-sm font-bold text-accent-700 dark:text-accent-300 mb-1">{tc(cat)}</h4>
              <table className="w-full text-sm">
                <tbody>
                  {rowsByCat[cat].map(({ item, closing }) => (
                    <tr key={item.id} className="border-t border-n-100">
                      <td className="py-1.5 pr-2 text-n-700">{ti(item.name, item)}</td>
                      <td className="py-1.5 px-1 text-right text-n-900 font-semibold w-20">
                        {closing}
                      </td>
                      <td className="py-1.5 pl-1 text-left text-n-400 w-12 text-xs">
                        {item.unit}
                      </td>
                    </tr>
                  ))}
...
function Stat({ label, value }) {
  return (
    <div className="text-center">
      <div className="text-xl font-bold text-n-900">{value}</div>
      <div className="text-[10px] uppercase tracking-wide text-n-400">{label}</div>
    </div>
  );
}

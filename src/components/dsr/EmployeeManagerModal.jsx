    ...
    >
      {editingId === emp.id ? (
        <input
          ...
          className="flex-1 min-w-0 bg-slate-100 border border-indigo-200 rounded-md px-2 py-1 text-sm text-slate-900 focus:outline-none"
        />
      ) : (
        <button
          type="button"
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-start sm:items-center justify-center p-4 overflow-y-auto">
      <div className="bg-slate-100 w-full max-w-md rounded-2xl border border-slate-200 shadow-xl my-8">
        ...
        <div className="p-4">
          ...
          <div className="flex gap-2 mb-4">
            <input
              ...
              className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500"
            />
            <button
              type="button"
              onClick={handleAdd}
              disabled={isAdding || !draft.trim()}
              className="px-3.5 py-2 rounded-lg text-sm font-semibold bg-indigo-600 hover:bg-indigo-500 text-white transition disabled:opacity-40"
            >
              {isAdding ? "…" : "Add"}
            </button>
          </div>
        ...
        <div className="flex justify-end p-4 border-t border-slate-200">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-lg text-sm font-semibold bg-indigo-600 hover:bg-indigo-500 text-white transition"
          >
            Done
          </button>
        </div>
